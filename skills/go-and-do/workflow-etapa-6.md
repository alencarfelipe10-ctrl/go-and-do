...
`detalhe` carries only a summary and the path after `lista inteira:` (t59/L10) — read THAT
file (or `extrai.pasta_suja`): it has every path, in three sections. Never stop at the
`detalhe` head (`commita-artefatos.sh` remains the only writer). Name, in the final banner,
